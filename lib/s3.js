...
async function select (params) {
  return new Promise((resolve, reject) => {
    s3.selectObjectContent(params, async (error, data) => {
      ...
      try {
        for await (const chunk of eventStream) { // eslint-disable-line
          if (chunk.Records && chunk.Records.Payload) buffer = Buffer.concat([buffer, chunk.Records.Payload])
        }
        const jsonString = buffer.toString('utf8')

        // Removing any terminating \n, then split into array
        // Do JSON.parse in try/catch to handle unexpected parse errors. This should rarely occur since
        // S3 sends all lines, even empty lines, as objects
